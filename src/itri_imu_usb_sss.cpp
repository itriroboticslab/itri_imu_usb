            ...
            try {
                ...
                while (buffer.size() >= 69) {
                    ...
                    if (buffer[0] == 0xFA && buffer[1] == 0xFF && 
                        ...
                        // Angular velocity section (ROS: rad/sec, ItriIMU: rad/sec)
                        if (data_ptr[0] == 0x80 && data_ptr[1] == 0x20 && data_ptr[2] == 0x0C) {
                            data_ptr += 3;  // Skip header
                            float roll_rad_sec = bigEndianToFloat(data_ptr);    data_ptr += 4;
                            float pitch_rad_sec = bigEndianToFloat(data_ptr);   data_ptr += 4;
                            float yaw_rad_sec = bigEndianToFloat(data_ptr);     data_ptr += 4;
                            // Convert to rad/sec
                            imu_msg.angular_velocity.x = roll_rad_sec;
                            imu_msg.angular_velocity.y = pitch_rad_sec;
                            imu_msg.angular_velocity.z = yaw_rad_sec;
                        } else {
                            buffer.erase(buffer.begin());
                            continue;
                        }
